                            ...
                            <table id="roles" class="table table-striped">
                                ...
                                @foreach(\App\Role::all() as $item)
                                    <tr>
                                        <td>{{$item['name']}}</td>
                                        <td class="d-flex">
                                            <a class="btn btn-flat btn-info ml-1" href="{{route('roles.show', $item)}}"><i class="fa fa-eye"></i></a>
                                            <a class="btn btn-flat btn-warning ml-1" href="{{route('roles.edit', $item)}}"><i class="fa fa-edit"></i></a>
                                            <form action="{{route('roles.destroy',$item)}}" method="post" onsubmit="return confirm('هل انت متاكد ؟')">
                                                @csrf
                                                @method('DELETE')
